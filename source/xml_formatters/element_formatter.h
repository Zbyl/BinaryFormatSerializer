...
#ifndef XmlFormatSerializer_element_formatter_H
#define XmlFormatSerializer_element_formatter_H
...
public:
    ...
    void save(XmlTree& xmlTree, const T& object) const
    {
        ...
        if (!element.hasName())
        {
            BOOST_THROW_EXCEPTION(xml_exception() << detail::errinfo_description("Element must be named."));
        }
    }
    ...
    void load(XmlTree& xmlTree, T& object) const
    {
        ...
        if (!element.isEmpty())
        {
            BOOST_THROW_EXCEPTION(xml_exception() << detail::errinfo_description("Element not consumed completely."));
        }
        if (!name && element.hasName())
        {
            BOOST_THROW_EXCEPTION(xml_exception() << detail::errinfo_description("Element name must be consumed."));
        }
    }
};
...
#endif // XmlFormatSerializer_element_formatter_H
