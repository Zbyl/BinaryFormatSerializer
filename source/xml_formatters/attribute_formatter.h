...
#ifndef XmlFormatSerializer_attribute_formatter_H
#define XmlFormatSerializer_attribute_formatter_H
...
public:
    ...
    void save(XmlTree& xmlTree, const T& object) const
    {
        ...
        {
            BOOST_THROW_EXCEPTION(xml_exception() << detail::errinfo_description("Attribute must be named."));
        }
    }
    ...
    void load(XmlTree& xmlTree, T& object) const
    {
        ...
        {
            BOOST_THROW_EXCEPTION(xml_exception() << detail::errinfo_description("Attribute value not consumed."));
        }
        if (!name && attribute.hasName())
        {
            BOOST_THROW_EXCEPTION(xml_exception() << detail::errinfo_description("Attribute name must be consumed."));
        }
    }
};
...
#endif // XmlFormatSerializer_attribute_formatter_H
